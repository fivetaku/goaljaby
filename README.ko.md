...
goaljaby는 PRD 폴더(수동 또는 `/show-me-the-prd` 결과물)를 받아 **한국어 검토 문서 5종** — VALIDATION, RECOVERY, PLAN, PROGRESS, `/goal` 본문 — 을 자동 생성한다. 한국어 검토 요약은 별도 파일로 만들지 않고 대화창에 바로 띄우고, 핸드오프 용도로 PROGRESS.md 상단에 4줄 요약을 prepend한다. 사람 승인 게이트를 거친 뒤 어시스턴트가 응답 마지막 줄로 `/goal`을 보내 다음 턴에서 골 작업을 시작한다. 한국어로 읽고, 한 번 승인하면, 작업이 진행된다.
...
- **검토 요약은 대화창에서 바로** — 별도 brief 파일을 만들지 않는다. Step 8에서 한국어 요약을 채팅에 띄우고, PROGRESS.md 상단에 4줄 요약을 prepend해 핸드오프 컨텍스트만 남긴다.
- **한 번 승인하면 작업이 시작** — 승인을 누르면 어시스턴트가 응답 마지막 줄로 `/goal {본문}`을 보내고, 다음 턴에서 세션이 그 골을 시작한다. 한국어 요약을 읽고 승인만 하면 작업이 진행된다.
...
[Step 7] 결정론적 자체 검증
     ...
     ▼
[Step 8] 한국어 검토 요약을 대화창에 표시
     │   + PROGRESS.md 상단에 4줄 요약 prepend (핸드오프용)
     ▼
...
├── PROGRESS.md        ← 빈 초기 템플릿 + Step 8 4줄 요약 prepended
└── goal-command.md    ← /goal 본문 (한국어, ≤4,000자)
```

5개 모두 한국어 위주. Step 8 검토 요약은 채팅에만 표시하고 별도 파일은 만들지 않는다. 파일명·명령어·shell 명령은 영어 그대로.
...
- **검토 요약은 채팅에만** — 별도 brief 파일은 만들지 않는다. PROGRESS.md 상단에 4줄 요약을 남겨 핸드오프 컨텍스트만 보존한다.
